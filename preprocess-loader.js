...
module.exports = function(source) {
        ...
        for(var x=0;x<opt.cfg.macros.length;x++) {
          if(opt.cfg.macros[x].scope=="line") {
            if(opt.cfg.macros[x].fileName=="all" || opt.cfg.macros[x].fileName==fileName) {
              line = line.replace(new RegExp(opt.cfg.macros[x].regex),opt.cfg.macros[x].value,opt.cfg.macros[x].flags);
            }
          }
        }
  ...
  if(opt.config!==undefined) {
    for(var i=0;i<opt.cfg.macros.length;i++) {
      if(opt.cfg.macros[i].scope=="source") {
        if(opt.cfg.macros[i].fileName=="all" || opt.cfg.macros[i].fileName==fileName) {
          content = content.replace(new RegExp(opt.cfg.macros[i].regex),opt.cfg.macros[i].replace-value,opt.cfg.macros[i].replace-scope);
        }
      }
    }
